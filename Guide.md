...
## 1. 定义有用的指标
### 1.1 直接测量你想要的东西
通常最佳做法是直接测量你真正关心的内容。例如，假设你想测量一个函数的运行时间，而该函数只是调用了两个子函数，且两个子函数都已经通过直方图打点。你可能会认为可以把这两个子函数的直方图相加得到总耗时，但这样会导致误导性的结果。若我们能知道每次打点的数据来自哪个调用，我们就可以配对计算总耗时。但直方图在客户端预聚合，这意味着无法恢复哪些打点应当被配对。把两个直方图相加等价于假设它们相互独立，但实际中未必如此。
...
通常不应轻易引入新的顶级类别。如果你想这样做，请先查看现有类别是否已覆盖你的指标。
...
选择最小值和最大值 最大值应保证很少有样本超过 max。若样本值 >= max，会落入“溢出桶”。若溢出桶过大，统计会困难。经验上，溢出桶占比最好 <= 1%（越小越好），这样可以分析 99 分位。宁可范围稍大，也不要太小。若设置不当，需要等下一个发布周期才能修复。 最小值：若关注全部可能值（含 0），请用 1。所有直方图都有 0 的下溢桶，因此 min=1 合适。否则用适合你场景的最小值。
...
例如，Feedback.ChromeOS.ExitPath它将是一个枚举直方图。
...
### 3.3 废弃直方图
废弃说明
在 histograms.xml 的直方图描述中，通过 expires_after 属性指定过期，该属性是必填项。可以用 YYYY-MM-DD 日期格式或 API(例如 API7.0.1) 里程碑格式。 过期后记录该直方图的代码就成为死代码，应该从代码库删除；同时也应清理 histograms.xml 的条目。 **极少数**情况下可以将过期设置为 **never"，用于标记关键指标（通常用于其他报告）。

常见场景建议
- 如果 owner 转到其他项目，找新的 owner。
- 如果 owner 和团队都不再使用该直方图，删除它。
- 如果当前不用、将来可能用，删除它。
...
## 4. 高性能打点
大多数情况下，您无需担心直方图打点的性能（我们测试的数据是小于1us）。
